...
int 研磨::Fun_ReadItemBuildUpListSelect()//强化界面当前选择装备。
{
	INT64 dUiAddr = UI功能::getUiObjById(0x12C);//&"root1.arkui.windowCanvas.itemBuildUpWnd"
	if (dUiAddr)
	{
...
//
BOOL 研磨::bCheckItemBuildUpLevelWndContentChildWindowShow(DWORD dChildIndex)
{
	INT64 dUiAddr = UI功能::getUiObjById(0x12C);//&"root1.arkui.windowCanvas.itemBuildUpWnd"
	if (dUiAddr)
	{
...
void Fun_ItemBuildUpLevelUp()
{
	...
	if (dUiAddr)
	{
		...
		if (dShow)
		{
			if (!研磨::bCheckItemBuildUpLevelWndContentChildWindowShow(1) && !研磨::bCheckItemBuildUpLevelWndContentChildWindowShow(0))//0参数是成长子窗口
			{
...
//
void Fun_ItemBuildUpChildMaxClick() //点击升级装备那个max按钮
{
	INT64 dUiAddr = UI功能::getUiObjById(0x12C);//&"root1.arkui.windowCanvas.itemBuildUpWnd"
	if (dUiAddr)
	{
...
//
void Fun_ItemBuildUpChildLevelUpBtnClick()//点击 研磨 升级按钮
{
	INT64 dUiAddr = UI功能::getUiObjById(0x12C);//&"root1.arkui.windowCanvas.itemBuildUpWnd"
	if (dUiAddr)
	{
...
void Fun_ItemBuildUpLevelUpGrade()
{
	...
	if (dUiAddr)
	{
		...
		if (dShow)
		{
			if (!研磨::bCheckItemBuildUpLevelWndContentChildWindowShow(1) && !研磨::bCheckItemBuildUpLevelWndContentChildWindowShow(0))//子界面1显示状态
			{
...
//
BOOL 研磨::bCheckitemBuildUpRenderTargetGroupShow()//判断查看研磨结果
{
	INT64 dUiAddr = UI功能::getUiObjById(0x12C);//&"root1.arkui.windowCanvas.itemBuildUpWnd"
	if (dUiAddr)
	{
...
BOOL 研磨::bCheckitemBuildUpSuccess_mc()//判断最终成功窗口
{
	INT64 dUiAddr = UI功能::getUiObjById(0x12C);//&"root1.arkui.windowCanvas.itemBuildUpWnd"
	if (dUiAddr)
	{
...
BOOL 研磨::bCheckitemBuildUpFail_mc()//判断最终失败窗口
{
	INT64 dUiAddr = UI功能::getUiObjById(0x12C);//&"root1.arkui.windowCanvas.itemBuildUpWnd"
	if (dUiAddr)
	{
...
void Fun_ItemBuildLevUpResult()
{
	...
	if (dUiAddr)
	{
		...
		if (dShow)
		{
			if (研磨::bCheckitemBuildUpRenderTargetGroupShow())
			{
...
//
void 研磨::Fun_ItemBuildLevUpSucess()//放在Fun_ItemBuildLevUpResult 后调用 中间加点延迟5000
{
	...
	if (dUiAddr)
	{
		...
		if (dShow)
		{
			if (研磨::bCheckitemBuildUpSuccess_mc() || 研磨::bCheckitemBuildUpFail_mc())
			{
...
//static void Fun_ItemBuildLevUpResult();//结果确定 点击   ok
...
bool 研磨::强化装备(int needuplv)
{
	ActorInfo_ 角色信息 = 本人::取角色信息();
	if (角色信息.装备评分 < needuplv)
	{
		if (环境::是否在和NPC对话())//需要升级装备
		{
			int 装备序号 = 背包::需研磨装备序号(needuplv);
			if (装备序号 == -1)
			{
				return true;
			}
			int 当前选择 = 研磨::Fun_ReadItemBuildUpListSelect();
			if (当前选择 != 装备序号)
			{
				Fun_ItemBuildUpListSelect(装备序号);
				Sleep(1000);
			}
			研磨::强化装备();
			return false;
