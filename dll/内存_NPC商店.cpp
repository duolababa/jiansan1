...
DWORD NPC商店::查询钻石数量()
{
	...
	//MyTrace(L"子数%d", num);
		...
		{
			double c=R_double(addr+i*0x11+9);
...
void NPC商店::openshop(vector<商城信息_>& vsk)
{
	...
	for (size_t i = 0; i < dtotal; i++)
	{
		...
		商城信息_ temp;
		temp.WearId = ID;
		//[[0x504DD80+lostark.0]+4e4]+0*218+192
		//INT64 dNameAddr = addr + i * g_商城循环 + 0x9;
		INT64 dNameAddr = addr + i * g_商城循环 + 0x192;
		CString csName = L"空";
		if (dNameAddr)
		{
			csName = R_CString(dNameAddr);
			temp.CName = csName;
			//MyTrace(L"CName%s", temp.CName);
		}
		vsk.push_back(temp);
...
DWORD 拍卖::市场遍历(DWORD ID)
{

	INT64 dItemResAddr = getItemResAddrById(ID);
	...
	if (csName != L"空")
	{
		...
		INT64 rcx1 = rcx+0x670;
		//MyTrace(L"rcx1%I64X", rcx1);
		//00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00
		W_QW(rcx1+8,0x0);
		W_QW(rcx1 + 10, 0x0);
		//UCHAR pBuff[0x100] = { 0 };
		W_QW(rcx1 + 0x18, 216180521644851210);
		W_QW(rcx1 + 0x20, 72059793128292352);
		W_QW(rcx1 + 0x28, 256186309936384);
		//W_BYTE(rcx1 + 0x30, 10);
		//W_DW(rcx1 + 0x1E, 671616);


		W_CString(rcx1 + 0x34, csName);



		MainUniversalCALL2(局_RCX, rcx1, 局_CALL);
		Sleep(3000);


		INT64 对象 = 拍卖_获取遍历对象(0x4EE4);

		if (对象 > 1)
		{
			MyTrace(L"对象0x%I64X ", 对象);
			for (size_t i = 0; i < 10; i++)
			{//4863C74869D0????????4883C2??
				// 000001B3E2AA0000+1E+0*3F0   价格   000001B855080000+1E+2*3F0+358
				INT64 ItemId = 对象 + g_市场遍历循环r + i * g_市场遍历循环头;

				DWORD ID1 = R_DW(ItemId + g_市场遍历拍卖id);

				if (ID == ID1)
				{
					DWORD 价格 = R_DW(ItemId + g_市场遍历最低价);
					return 价格;
...
void NPC商店::shoppingmall(vector<商城信息_>& vsk)
{
	...
	if (dtotal > 1 && dtotal < 100)
	{
		...
		for (size_t i = 0; i < dtotal; i++)
		{
			...
			//	MyTrace(L"addr_shop 对象0x%I64X", addr_shop);
				for (size_t b = 0; b < dNum; b++)
				{
					商城信息_ temp;
					temp.ItemId = ItemId;
					temp.WearId = R_DW(addr_shop + b * 0x10);
					//MyTrace(L"ID%d", temp.WearId);
...
INT64 拍卖::拍卖_获取遍历对象(DWORD ID)
{
	INT64 rcx = R_QW(游戏模块 + 基址_拍卖_获取遍历对象rcx);
	INT64 ret = MainUniversalCALL4_Ret(rcx, ID, 0, 0, 游戏模块 + 交易金额call);
...
{
	INT64 rcx = R_QW(游戏模块 + gb_ExpressMailList);
	//MyTrace(L"rcx0x%I64X ",rcx);
	DWORD 数量 = R_DW(rcx + g_邮寄数量);
	return 数量;
}
